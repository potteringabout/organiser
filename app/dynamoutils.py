...
class DynamoDBHelper:
    ...
    def query_gsi(self, gsi_name, k1, v1, k2, v2=None, startswith=False):
        ...
        response = self.table.query(
            IndexName=gsi_name, KeyConditionExpression=key_condition
        )
    ...
    def upsert(self, item):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to create item: {e.response['Error']['Message']}"
            )
    ...
    def batch_get(self, keys):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to retrieve items: {e.response['Error']['Message']}"
            )
        # Example usage:
    ...
    def batch_update(self, items):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to update items: {e.response['Error']['Message']}"
            )
    ...
    def batch_delete(self, keys):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to delete items: {e.response['Error']['Message']}"
            )
...
def dynamodb_to_json(dynamodb_object):
    ...
    return {
        key: deserializer.deserialize(value) for key, value in dynamodb_object.items()
    }
