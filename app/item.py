...
def delete_note(board_id, note_id):   
  try:
    dynamo.batch_delete([{"EntityType" : "BoardItem", "ID": f"{board_id}-{note_id}"},
...
def get_notes(board_id):
  ...
  try:
    r = dynamo.query_gsi(gsi_name="OwnerItemID", k1="Owner", v1=user["user_id"], k2="ID", v2=f"{board_id}-", startswith=True)
