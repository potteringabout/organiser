...
def create_task(user, board_id, text, due_date):
    """
    Create a note with associated tags and tasks in DynamoDB.
    ...
    try:
        r = dynamo.batch_update([task.to_dict(), link.to_dict()])
        return r
    except RuntimeError as e:
        return {"error": str(e)}
...
def delete_task(board_id, task_id):
    try:
        dynamo.batch_delete(
            [
                {"EntityType": "BoardItem", "ID": f"{board_id}-{task_id}"},
                {"EntityType": "Note", "ID": task_id},
            ]
        )
        return {"success": True}
    except RuntimeError as e:
...
def get_task(board_id, task_id):
    try:
        r = dynamo.get("BoardItem", f"{board_id}-{task_id}")
        return r
    except RuntimeError as e:
        return {"error": str(e)}
...
def get_tasks(board_id):
    ...
    try:
        ...
        keys = []
        for item in r:
            task_id = "Note-" + item["ID"].split("Task-", 1)[1]
            keys.append({"EntityType": "Task", "ID": task_id})
